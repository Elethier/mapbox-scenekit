...
internal final class ImageBuilder {
    ...
    private let imageBuilderQueue = DispatchQueue(label: "com.mapbox.SceneKit.imageBuilder")
    ...
    func addTile(x: Int, y: Int, image: UIImage) {
        imageBuilderQueue.sync { [weak self] in
            guard let strongSelf = self else { return }
            strongSelf.context?.draw(image.cgImage!, in: CGRect(origin: CGPoint(x: CGFloat(x) * strongSelf.tileSize.width, y: CGFloat(Int(strongSelf.imageSize.height / strongSelf.tileSize.height) - y - 1) * strongSelf.tileSize.height), size: strongSelf.tileSize))
        }
    }
